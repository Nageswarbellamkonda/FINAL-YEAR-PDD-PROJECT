...
BEGIN
  ...
  -- Monthly Trend Data
  SELECT jsonb_agg(jsonb_build_object('month', m, 'filed', filed_c, 'resolved', resolved_c))
  INTO monthly_trend_data
  FROM (
    SELECT to_char(created_at, 'MMM') as m, 
           COUNT(*) as filed_c,
           COUNT(*) FILTER (WHERE status IN ('resolved', 'closed')) as resolved_c
    FROM public.complaints
    WHERE created_at >= CURRENT_DATE - interval '6 months' AND (p_district IS NULL OR p_district = 'All' OR district = p_district)
    GROUP BY to_char(created_at, 'MMM'), extract(month from created_at)
  ...
  res := jsonb_build_object(
    ...
    'monthlyTrendData', COALESCE(monthly_trend_data, '[]'::jsonb),
